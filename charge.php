...
if (strcasecmp($letab,"sourcescompetitions")==0) {
	if (is_file($chemin_data.$letab."/".$lelogin."_".$lepass."_".$lavariante.".txt")) {
		...
		echo $json;
		file_put_contents($chemin_data."logs_load.txt", strftime("Le %A %d %B à %H:%M:%S")." -> Chargement réussi du code Compétition ".$lelogin." depuis l'IP ".getenv('REMOTE_ADDR')." avec ".$_SERVER[ 'HTTP_USER_AGENT']."\r\n",FILE_APPEND); 
	} else {
		print "1";
		file_put_contents($chemin_data."logs_load.txt", strftime("Le %A %d %B à %H:%M:%S")." -> ECHEC de chargement du code Compétition ".$lelogin." depuis l'IP ".getenv('REMOTE_ADDR')." avec ".$_SERVER[ 'HTTP_USER_AGENT']."\r\n",FILE_APPEND); 
	}
} else {
